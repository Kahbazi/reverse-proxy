...
namespace Microsoft.ReverseProxy.Service.RuntimeModel.Transforms
{
    ...
    public class RequestHeaderForwardedTransform : RequestTransform
    {
        ...
        private void AppendNode(IPAddress ipAddress, int port, NodeFormat format, StringBuilder builder)
        {
            ...
            // character in "token"."
            var addPort = port != 0 && (format == NodeFormat.IpAndPort || format == NodeFormat.UnknownAndPort || format == NodeFormat.RandomIpAndPort);
            var addRandomPort = (format == NodeFormat.IpAndRandomPort || format == NodeFormat.UnknownAndRandomPort || format == NodeFormat.RandomIpAndRandomPort);
            ...
            switch (format)
            {
                ...
                case NodeFormat.UnknownAndRandomPort:
                    ...
                    break;
                case NodeFormat.RandomIp:
                case NodeFormat.RandomIpAndPort:
                case NodeFormat.RandomIpAndRandomPort:
